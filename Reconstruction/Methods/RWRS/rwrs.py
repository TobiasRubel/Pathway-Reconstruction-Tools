...
##the same as RWR, but only from sources 
##TODO: add comments
...
def run(G, sources, targets, alpha=0.85, thres=0.80, verbose=False):
	...
	tot_flux = 0
	for u,v in G.edges():
		if G[u][v]['forward_flux'] == 0:
			G[u][v]['neglog_forward_flux'] = sys.float_info.max ## maximum float value
		else:
			G[u][v]['neglog_forward_flux'] = - (math_log(G[u][v]['forward_flux'])) 
			tot_flux+=G[u][v]['forward_flux']
	...
	for u,v,d in sorted(G.edges(data=True), key=lambda t: t[2]['neglog_forward_flux']):
		edges[(u,v)] = G[u][v]['neglog_forward_flux']
		running_sum+=G[u][v]['forward_flux']
		counter+=1
		if counter % 1000 == 0:
...
def main(argv):
	...
	write_output(prediction,'RWRS.csv',verbose=verbose)
