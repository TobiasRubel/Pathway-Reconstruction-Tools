...
def main(argv):
    ...
    #do first run
    print(os.getcwd())
    os.chdir('Methods/PCSF')
    network,sources,dummy_edge_weight,edge_reliability,degree_penalty,prize,verbose = argv[1],argv[2],*argv[3:]
    #we need to modify network,pathway paths
    network = os.path.join('../../',network)
    sources = os.path.join('../../',sources)
    argv = ['python3','pcsf.py']+[network,sources,dummy_edge_weight,edge_reliability,degree_penalty,prize,verbose]
    print(argv)
    subprocess.call(argv)
    #generate new interactome
    print(os.listdir('.'))
    #os.remove(next(x for x in os.listdir('.') if '.lp' in x))
    pedges = next(x for x in os.listdir('.') if '.csv' in x)
    ...
    graph_to_file(H)
    #do second run
    os.chdir('../PerfectLinker')
    print('Doing second run')
    argv_2 = ['python3','PL.py','nodes',network,'../PCSF/tmp-interactome.csv',sources]
    subprocess.call(argv_2)
    os.replace('nodes-PerfectLinker.csv','../../HybridLinker-PCSF.csv')
