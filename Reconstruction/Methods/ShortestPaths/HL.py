...
def main(argv):
    ...
    #do first run
    print(os.getcwd())
    os.chdir('Methods/ShortestPaths')
    os.system('rm shortest_paths.csv')
    ...
    argv = ['python3','shortest_paths.py']+[network,sources,'False']
    print(argv)
    subprocess.call(argv)
    #generate new interactome
    pedges = next(x for x in os.listdir('.') if 'shortest_paths.csv' in x)
    ...
    argv_2 = ['python3','PL.py','nodes',network,'../ShortestPaths/tmp-interactome.csv',sources]
    subprocess.call(argv_2)
    os.replace('nodes-PerfectLinker.csv','../../HybridLinker-SP.csv')
