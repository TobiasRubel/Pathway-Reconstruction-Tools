...
def main(argv):
    ...
    #do first run
    print(os.getcwd())
    os.chdir('Methods/ResponseNet')
    #os.system('rm bowtie_builder.csv')
    network,sources,gamma = argv[1],argv[2],argv[3]
    #we need to modify network,pathway paths
    network = os.path.join('../../',network)
    sources = os.path.join('../../',sources)
    argv = ['python3','response_net.py']+[network,sources,gamma,'False']
    print(argv)
    subprocess.call(argv)
    #generate new interactome
    print(os.listdir('.'))
    os.remove(next(x for x in os.listdir('.') if '.lp' in x))
    pedges = next(x for x in os.listdir('.') if 'response_net_gamma_{}'.format(gamma) in x)
    ...
    argv_2 = ['python3','PL.py','nodes',network,'../ResponseNet/tmp-interactome.csv',sources]
    subprocess.call(argv_2)
    os.replace('nodes-PerfectLinker.csv','../../HybridLinker-RN.csv')
