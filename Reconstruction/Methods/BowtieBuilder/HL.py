...
def main(argv):
    ...
    #do first run
    print(os.getcwd())
    os.chdir('Methods/BowtieBuilder')
    os.system('rm bowtie_builder.csv')
    network,sources, = argv[1],argv[2],
    #we need to modify network,pathway paths
    network = os.path.join('../../',network)
    sources = os.path.join('../../',sources)
    argv = ['python3','bowtie_builder.py']+[network,sources,'False']
    print(argv)
    subprocess.call(argv)
    #generate new interactome
    pedges = next(x for x in os.listdir('.') if 'bowtie_builder.csv' in x)
    ...
    argv_2 = ['python3','PL.py','nodes',network,'../BowtieBuilder/tmp-interactome.csv',sources]
    subprocess.call(argv_2)
    os.replace('nodes-PerfectLinker.csv','../../HybridLinker-BTB.csv')
